...
export class MemoryStore {
  ...
  ensureMemoryStatusSchema() {
    ...
    try {
      this.db.exec(`
        ...
        INSERT INTO memories_migrated (id, directory_id, body, status, content_hash, created_at, updated_at, last_recalled_at)
          SELECT id, directory_id, body, CASE WHEN status = 'active' THEN 'active' ELSE 'inactive' END, content_hash, created_at, updated_at, last_recalled_at FROM memories;
        DROP TABLE memories;
  ...
  createMemory(input = {}, options = {}) {
    ...
    try {
      ...
      this.db.prepare('INSERT INTO memories (id, directory_id, body, status, content_hash, created_at, updated_at, last_recalled_at) VALUES (?, ?, ?, ?, ?, ?, ?, 0)')
  ...
  persistMemory(input = {}, options = {}) {
    ...
    try {
      ...
      const stamp = now();
      const persistentId = id('mem');
      /* 临时记忆转正：last_recalled_at 随迁（保留该临时记忆历史上的最新召回时间） */
      this.db.prepare('INSERT INTO memories (id, directory_id, body, status, content_hash, created_at, updated_at, last_recalled_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)')
        .run(persistentId, directoryId, body, normalizeStatus(input.status, 'active'), hash, stamp, stamp, Number(item.lastRecalledAt) || 0);
