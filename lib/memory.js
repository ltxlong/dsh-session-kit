...
function truncateForActivity(value, maxLength = 180) {
  const text = normalizePlainText(value, maxLength + 1);
...
function formatDistilledContent(payload, transcriptText = '') {
  const content = normalizePlainText(payload?.content ?? payload?.summary ?? payload?.text ?? payload?.memory ?? '', Number.MAX_SAFE_INTEGER);
  if (content.length < 6) return '';
  if (!shouldUseStructuredContent(payload, transcriptText)) return content;
  const paths = normalizeDistillList(payload?.paths);
  const symbols = normalizeDistillList(payload?.symbols);
...
function normalizeDistilledMemoryText(text, transcriptText = '') {
  const content = normalizePlainText(text, Number.MAX_SAFE_INTEGER);
  if (content.length < 6) return '';
...
function parseDistillation(text, transcriptText) {
  ...
  /* 不做整条截断：组件级上限（content ≤1500、paths/symbols 数量与单条上限）+
     LLM 输出上限已约束体积；上下文体积由注入侧单条限额保证 */
...
/* 结构感知限额（注入侧专用核心）：结构化记忆的“位置/对象”召回键行完整保留，
   “内容”正文超限截断；非结构化记忆整条同规则。返回 { text, truncated }。 */
function limitStructuredMemoryText(text, contentChars) {
  const lines = text.split('\n');
  if (lines.length >= 3 && lines[0].startsWith('位置：') && lines[1].startsWith('对象：') && lines[2].startsWith('内容：')) {
    const contentStart = lines[0].length + 1 + lines[1].length + 1 + 3; /* 跳过“内容：”前缀 */
    const content = text.slice(contentStart);
    if (content.length <= contentChars) return { text, truncated: false };
    return {
      text: text.slice(0, contentStart + contentChars),
      truncated: true
    };
  }
  if (text.length <= contentChars) return { text, truncated: false };
  return { text: text.slice(0, contentChars), truncated: true };
}

/* 注入侧包装：限额之上追加 id + memory_read 提示，模型可按需获取全文。 */
...
function packHits(rows, maxItems = RECALL_MAX_ITEMS) {
  /* v9：四段席位已在上游选定，条数即数量边界；
     单条体积由注入截断保证（召回键行全保留、内容限额 + memory_read 提示）。 */
  const hits = [];
...
export class MemoryStore {
  ...
  createDirectory(input = {}) {
    const activity = this.beginActivity('directory_create', { sessionTitle: '', summary: truncateForActivity(input.name, 180), triggerKind: 'manual' });
    try {
      const directoryId = this.ensureDirectory(input.name, input.autoSessionIds);
      this.finishActivity(activity, 'success', { summary: truncateForActivity(this.db.prepare('SELECT name FROM memory_directories WHERE id = ?').get(directoryId)?.name || input.name, 180) });
    } catch (error) {
  ...
  updateDirectory(input = {}) {
    ...
    const activity = this.beginActivity('directory_update', { sessionTitle: '', summary: truncateForActivity(summary, 180), triggerKind: 'manual' });
    try {
      ...
      this.startFtsRebuild();
      this.finishActivity(activity, 'success', { summary: truncateForActivity(summary, 180) });
    } catch (error) {
      this.finishActivity(activity, 'failed', { errorCode: error?.code, errorMessage: error?.message || String(error) });
      throw error;
    }
  ...
  deleteDirectory(input = {}) {
    ...
    const activity = this.beginActivity('directory_delete', { sessionTitle: '', summary: truncateForActivity(directory.name, 180), triggerKind: 'manual' });
    try {
  ...
  createTag(input = {}) {
    ...
    const activity = this.beginActivity('tag_create', { sessionTitle: '', summary: truncateForActivity(name, 180), triggerKind: 'manual' });
    try {
  ...
  setTagStatus(input = {}) {
    ...
    const activity = this.beginActivity('tag_update', { sessionTitle: '', summary: truncateForActivity(summary, 180), triggerKind: 'manual' });
    try {
  ...
  deleteTag(input = {}) {
    ...
    const activity = this.beginActivity('tag_delete', { sessionTitle: '', summary: truncateForActivity(row.name, 180), triggerKind: 'manual' });
    try {
  ...
  createMemory(input = {}, options = {}) {
    const activity = options.logActivity === true ? this.beginActivity('memory_add', { sessionId: '', sessionTitle: '', summary: truncateForActivity(input.text, 180), triggerKind: 'manual' }) : null;
    try {
      const directoryId = normalizePlainText(input.directoryId, 128);
      const directory = this.db.prepare('SELECT id FROM memory_directories WHERE id = ?').get(directoryId);
      if (directory === undefined) throw new MemoryError('directory-not-found', 'directory was not found', 404);
      const body = normalizePlainText(input.text, Number.MAX_SAFE_INTEGER);
      ...
      if (duplicate !== undefined) {
        this.finishActivity(activity, 'duplicate', { memoryId: duplicate, summary: truncateForActivity(body, 180) });
        return duplicate;
      }
      const stamp = now();
      ...
      this.finishActivity(activity, 'success', { memoryId: persistentId, summary: truncateForActivity(body, 180) });
      return persistentId;
    } catch (error) {
      this.finishActivity(activity, 'failed', { errorCode: error?.code, errorMessage: error?.message || String(error) });
      throw error;
  ...
  updateMemory(input = {}, options = {}) {
    ...
    if (ephemeral !== undefined) {
      try {
        if (input.text !== undefined) {
          const text = normalizePlainText(input.text, Number.MAX_SAFE_INTEGER);
        ...
        this.finishActivity(manualActivity, 'success', { memoryId, summary: truncateForActivity(ephemeral.text || '', 180) });
      } catch (error) {
    ...
    try {
      const body = input.text === undefined ? row.body : normalizePlainText(input.text, Number.MAX_SAFE_INTEGER);
      if (body.length < 6) throw new MemoryError('invalid-memory-text', 'memory text must not be empty');
      const hash = memoryHash(body);
      const duplicate = this.persistentByHash(hash);
      ...
      this.finishActivity(manualActivity, 'success', { memoryId, summary: truncateForActivity(body, 180) });
    } catch (error) {
      this.finishActivity(manualActivity, 'failed', { memoryId, errorCode: error?.code, errorMessage: error?.message || String(error) });
      throw error;
    }
  ...
  deleteMemory(input = {}) {
    ...
    const ephemeral = this.ephemeral.get(memoryId);
    if (ephemeral !== undefined) {
      const activity = this.beginActivity('memory_delete', { memoryId, sessionId: String(ephemeral.sourceSessionId || ''), sessionTitle: String(ephemeral.sourceSessionTitle || ''), summary: truncateForActivity(ephemeral.text || '', 180), triggerKind: 'manual' });
      try {
        this.ephemeral.delete(memoryId);
        this.finishActivity(activity, 'success');
      } catch (error) {
    ...
    const activity = this.beginActivity('memory_delete', { memoryId, sessionTitle: '', summary: truncateForActivity(old.body || '', 180), triggerKind: 'manual' });
    try {
      ...
      this.finishActivity(activity, 'success', { memoryId, summary: truncateForActivity(old.body || '', 180) });
    } catch (error) {
  ...
  async distillTurn(session, turn, modelSelection, triggerKind = 'manual') {
    ...
    const activity = this.beginActivity('distill', { sessionId: String(session?.id ?? ''), sessionTitle: sessionTitle(session), turn, summary: truncateForActivity(firstUserText, 180), triggerKind });
    try {
  ...
  /* 总览聚合：今天/本周/本月/最近四个窗口（本地时区边界），每窗口返回
     tok 总量与模型/类型/项目三个维度的占比（均为 token 份额，单查询取行 JS 聚合） */
  usageOverview() {
    ...
    const directoryNames = new Map(this.db.prepare('SELECT id, name FROM memory_directories').all().map((row) => [row.id, row.name]));
    const windowFor = (fromMs) => {
      const rows = this.db.prepare('SELECT kind, directory_id, model, tokens FROM memory_token_usage WHERE created_at >= ? AND tokens IS NOT NULL').all(fromMs);
      ...
      for (const row of rows) {
        ...
        kinds.set(row.kind || '', (kinds.get(row.kind || '') || 0) + tokens);
        projects.set(row.directory_id || '', (projects.get(row.directory_id || '') || 0) + tokens);
      }
      ...
      return {
        ...
        kinds: ranked(kinds, () => ({})),
        projects: ranked(projects, (key) => ({ directoryId: key, name: directoryNames.get(key) ?? null }))
      };
    };
    return {
      today: windowFor(startOfDay.getTime()),
      week: windowFor(startOfWeek.getTime()),
      month: windowFor(startOfMonth.getTime()),
      recent: windowFor(startOfRecent)
    };
  }
  ...
  addToolMemory(input = {}, sessionId = '', activityContext = {}) {
    ...
    const activity = this.beginActivity('memory_add', { sessionId: '', sessionTitle: '', turn: activityContext.turn, callId: activityContext.callId, summary: truncateForActivity(userSummary, 180), triggerKind: 'tool' });
    const body = normalizePlainText(input.text ?? input.content, Number.MAX_SAFE_INTEGER);
    if (body.length < 6) {
  ...
  updateToolMemory(input = {}, sessionId = '') {
    ...
    const activity = this.beginActivity('memory_update', { sessionId: '', sessionTitle: '', memoryId, summary: truncateForActivity(before.text || `memory ${memoryId}`, 180), triggerKind: 'tool' });
    ...
    const run = () => {
      try {
        ...
        const memory = this.memoryById(memoryId);
        if (memory === undefined) throw new MemoryError('memory-not-found', 'memory was not found', 404);
        this.finishActivity(activity, 'success', { memoryId, summary: normalizePlainText(memory.text, 180) });
      } catch (error) {
        this.finishActivity(activity, 'failed', { memoryId, errorCode: error?.code, errorMessage: error?.message || String(error) });
        throw error;
      }
