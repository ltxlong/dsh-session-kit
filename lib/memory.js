...
function formatDistilledContent(payload, transcriptText = '') {
  const content = normalizePlainText(payload?.content ?? payload?.summary ?? payload?.text ?? payload?.memory ?? '', 1500);
...
function scoreRowComponents(row, profile, startedAt, queryText) {
...
function rankRowsBySegment(rows, segment, profile, startedAt, queryText) {
  const keys = SEGMENT_COMPONENT_KEYS[segment] ?? SEGMENT_COMPONENT_KEYS.fallback;
  return rows
    .map((row) => {
      const parts = scoreRowComponents(row, profile, startedAt, queryText);
      let finalRank = 0;
...
function rerankRecallRows(rows, profile, startedAt = now(), queryText = '') {
  ...
  return rankRowsBySegment(rows, 'mixed', profile, startedAt, queryText)
    .map(({ row, parts }) => ({
      ...row,
...
function packHits(rows, maxItems = RECALL_MAX_ITEMS, maxChars = RECALL_MAX_CHARS) {
  /* v9：四段席位已在上游选定，pack 只做字符预算收尾。
     放不下的条目跳过并继续（避免单条大记忆截断尾部段）；仍按上游顺序保序。 */
  const hits = [];
  let used = 0;
  for (const row of rows) {
    ...
    const minLen = Math.min(item.text.length, remaining);
    const truncated = item.text.length > remaining;
    const text = truncated ? `${item.text.slice(0, Math.max(80, minLen - 24))}\n…（该记忆已截断）` : item.text;
...
export class MemoryStore {
  ...
  addEphemeralMemory({ text, tags, session, sourceTurn, distillUsage, directoryId }) {
    ...
    return { status: 'created', memoryId: item.id };
  }
  ...
  ephemeralSegmentRows(sessionId, enabledSet, terms, customContext, profileWeights, allowProfileTagFallback, activeTagNames, candidateLimit) {
  ...
  recall(sessionId, queryText, options = {}) {
    ...
    const candidateLimit = Math.max(RECALL_MAX_CANDIDATES, maxItems * 8);
    const customContext = customTagMatchContext(queryText);
    ...
    const ephemeralRows = this.ephemeralSegmentRows(sessionIdText, enabledSet, terms, customContext, profileWeights, allowProfileTagFallback, activeTagNames, candidateLimit);
    let customRows;
    const getCustomRows = () => {
    ...
    /* 候选收集：FTS 主路 + LIKE 兜底路，产出统一的三池划分 */
    let permanentRows = [];
    ...
    permanentRows = mergeRecallRows(permanentRows, getCustomRows());
    /* 三池划分 */
    const bottomPool = [];
    const regularPool = [];
    const seen = new Set();
    ...
    /* 四段构建：每段用本段矩阵竞争选拔 */
    const segment = (poolRows, segmentKey, count) => rankRowsBySegment(poolRows, segmentKey, profile, startedAt, queryText).slice(0, count).map((entry) => entry.row);
    ...
    return packHits(combined, maxItems, options.maxChars ?? RECALL_MAX_CHARS);
  }
  ...
  async distillTurn(session, turn, modelSelection, triggerKind = 'manual') {
    ...
    try {
      const result = await this.distillTurnCore(session, turn, modelSelection);
  ...
  async distillTurnCore(session, turn, modelSelection) {
    const override = this.distillModelOverride();
    const primary = override ?? modelSelection;
    const primaryResult = await this.distillTurnAttempt(session, turn, primary);
    ...
    const fallbackResult = await this.distillTurnAttempt(session, turn, fallback);
  ...
  async distillTurnAttempt(session, turn, modelSelection) {
    const rows = turnTranscript(session, turn);
