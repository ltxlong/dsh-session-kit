window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const DEFAULT_SIDEBAR_ENTRIES = Object.freeze({ memoryVisible: true, archiveVisible: true, taskVisible: true });
    ...
    const zh = {
      ...
      memoryRecallSegment2Forced: '最多15条时，段2自动为0',
      memoryRecallSegment3Forced: '最多10条时，段3自动为0',
    ...
    const en = {
      ...
      taskSearchPlaceholder: 'Search tasks or projects',
      ...
      memoryRecallSegment2Forced: 'Segment 2 is automatically 0 at totals up to 15',
      memoryRecallSegment3Forced: 'Segment 3 is automatically 0 at totals up to 10',
    ...
    function TaskIcon({ size = 16 }) {
      return react.createElement(primitives.IconListPenOutline16, { size });
    }
    function BranchIcon() {
    ...
    function MemoryManagementDialog({ open, t, sessionId, onClose, getSessionTitle, getCurrentUserMessage, getModelSelection, getModelDirectory, updateSidebarEntries }) {
      ...
      const recallCustomValid = recallSettings.mode !== 'custom' || (recallSettings.maxItems >= 8 && recallSettings.maxItems <= 20 && recallSettings.segments.segment1 >= 3 && recallSettings.segments.segment1 <= 5 && recallSettings.segments.segment2 >= 0 && recallSettings.segments.segment2 <= 5 && recallSettings.segments.segment3 >= 0 && recallSettings.segments.segment3 <= 5 && recallSettings.segments.segment4 >= 3);
      const setRecallMode = (mode) => {
        recallSettingsDirtyRef.current = true;
        if (mode === 'default') setRecallSettingsDraft({ mode, maxItems: 20, segments: { segment1: 5, segment2: 5, segment3: 5, segment4: 5 } });
        else if (mode === 'exclude-temporary') setRecallSettingsDraft({ mode, maxItems: 15, segments: { segment1: 5, segment2: 0, segment3: 5, segment4: 5 } });
        else setRecallSettingsDraft({ ...recallSettings, mode: 'custom' });
      };
      const setRecallNumber = (key, value) => {
        recallSettingsDirtyRef.current = true;
        const segments = { ...recallSettings.segments, [key]: Math.max(key === 'segment1' ? 3 : 0, Math.min(5, Math.trunc(Number(value) || 0))) };
        const segment4 = Math.max(0, recallSettings.maxItems - segments.segment1 - segments.segment2 - segments.segment3);
        setRecallSettingsDraft({ ...recallSettings, mode: 'custom', segments: { ...segments, segment4 } });
      };
      const setRecallMax = (value) => {
        recallSettingsDirtyRef.current = true;
        const previousMax = recallSettings.maxItems;
        const maxItems = Math.max(8, Math.min(20, Math.trunc(Number(value) || 8)));
        let segment2 = recallSettings.segments.segment2;
        let segment3 = recallSettings.segments.segment3;
        ...
        if (maxItems <= 15) segment2 = 0;
        if (maxItems <= 10) segment3 = 0;
        const segments = { ...recallSettings.segments, segment2, segment3 };
        const segment4 = Math.max(0, maxItems - segments.segment1 - segment2 - segment3);
        setRecallSettingsDraft({ ...recallSettings, mode: 'custom', maxItems, segments: { ...segments, segment4 } });
      };
      const renderRecallSlider = (min, max, value, disabled, onChange, ariaLabel) => {
      ...
      const renderMemory = (memory) => {
        ...
        return react.createElement('article', { key: `${isPersisted ? 'stored' : 'tmp'}:${memory.id}`, className: `dsh-session-kit-memory-card${isPersisted ? '' : ' dsh-session-kit-memory-card-ephemeral'}` },
          react.createElement('div', { className: 'dsh-session-kit-memory-card-top' },
            ...
            react.createElement('div', { className: 'dsh-session-kit-memory-card-actions' },
              react.createElement(primitives.Button, { variant: 'outline', size: 'sm', disabled: busy || !currentSessionId, onClick: () => setMemoryInjectTarget(memory) }, t('memoryInject')),
      ...
      const renderTabPanel = () => {
        ...
        if (memoryTab === 'settings') return react.createElement('div', { className: 'dsh-session-kit-memory-settings dsh-session-kit-memory-panel' },
          ...
          react.createElement('div', { className: 'dsh-session-kit-memory-recall' },
            react.createElement('div', { className: 'dsh-session-kit-memory-section-title' }, t('memoryRecallSettings')),
            react.createElement('p', { className: 'dsh-session-kit-compaction-desc' }, t('memoryRecallSettingsDesc')),
            react.createElement('p', { className: 'dsh-session-kit-memory-recall-note' }, t('memoryRecallDedupNote')),
            react.createElement('p', { className: 'dsh-session-kit-memory-recall-note' }, t('memoryRecallOrderNote')),
            react.createElement('label', { className: 'dsh-session-kit-global-prompt-field' },
              react.createElement('span', null, t('memoryRecallMode')),
              react.createElement(MemorySelect, {
                value: recallSettings.mode,
                options: [
            ...
            react.createElement('div', { className: 'dsh-session-kit-memory-recall-field' },
              react.createElement('div', { className: 'dsh-session-kit-memory-recall-field-head' }, react.createElement('span', null, t('memoryRecallMax')), react.createElement('strong', { className: recallSettings.mode === 'custom' ? 'dsh-session-kit-memory-recall-custom-value' : undefined }, recallSettings.maxItems)),
              recallSettings.mode === 'custom' && renderRecallSlider(8, 20, recallSettings.maxItems, busy || loading, (event) => setRecallMax(event.currentTarget.value), t('memoryRecallMax')),
              recallSettings.mode === 'custom' && react.createElement('input', { className: 'dsh-session-kit-memory-recall-number', type: 'number', min: 8, max: 20, step: 1, value: recallSettings.maxItems, disabled: busy || loading, onChange: (event) => setRecallMax(event.currentTarget.value) })
            ),
            react.createElement('div', { className: 'dsh-session-kit-memory-recall-segments' },
              [['segment1', 'memoryRecallSegment1', 3], ['segment2', 'memoryRecallSegment2', 0], ['segment3', 'memoryRecallSegment3', 0], ['segment4', 'memoryRecallSegment4', 0]].map(([key, labelKey, min]) => {
                const value = recallSettings.segments[key];
                const editable = recallSettings.mode === 'custom' && key !== 'segment4';
                    ...
                    react.createElement('span', { className: 'dsh-session-kit-memory-recall-label' },
                      ...
                      react.createElement(primitives.Tooltip, { label: t(`${labelKey}Hint`), side: 'top', delayMs: 300 },
                        react.createElement('span', { className: 'dsh-session-kit-memory-recall-hint-icon', role: 'img', 'aria-label': t(`${labelKey}Hint`), tabIndex: 0 },
                          react.createElement(primitives.IconWarningOutline16, { size: 14 })
                        )
                      )
                    ),
                    react.createElement('strong', { className: recallSettings.mode === 'custom' ? 'dsh-session-kit-memory-recall-custom-value' : undefined }, value)),
                  editable && renderRecallSlider(min, 5, value, busy || loading || (key === 'segment2' && recallSettings.maxItems <= 15) || (key === 'segment3' && recallSettings.maxItems <= 10), (event) => setRecallNumber(key, event.currentTarget.value), t(labelKey)),
                  key === 'segment2' && recallSettings.mode === 'custom' && recallSettings.maxItems <= 15 && react.createElement('small', null, t('memoryRecallSegment2Forced')),
                  key === 'segment3' && recallSettings.mode === 'custom' && recallSettings.maxItems <= 10 && react.createElement('small', null, t('memoryRecallSegment3Forced')),
                  key === 'segment4' && react.createElement('small', null, t('memoryRecallSegmentDynamic'))
                );
              })
            ),
            !recallCustomValid && react.createElement('div', { role: 'alert', className: 'dsh-session-kit-memory-form-error' }, t('memoryRecallInvalid')),
            react.createElement(primitives.Button, { variant: 'outline', className: 'dsh-session-kit-memory-settings-save', disabled: busy || loading || !recallCustomValid, onClick: () => { recallSettingsDirtyRef.current = false; void runAction({ action: 'set-recall-settings', recallSettings }, t('memorySaved')); } }, t('memorySave'))
          )
        );
      ...
      const confirmMemoryInject = async () => {
        if (memoryInjectTarget === null || busy) return;
    ...
    function TaskManagerDialog({ open, t, sessionId, onClose, updateSidebarEntries, getSessionTitle }) {
      const sidebarEntries = useSidebarEntries();
      const [loading, setLoading] = react.useState(false);
        ...
        return react.createElement(
          ...
          react.createElement(
            ...
            react.createElement('button', {
              ...
              react.createElement(expanded ? primitives.IconChevronUpOutline14 : primitives.IconChevronDownOutline14, { size: 14, className: 'dsh-session-kit-task-stat-chevron' })
            ),
            ...
            react.createElement('button', {
              ...
              react.createElement(sessionsExpanded ? primitives.IconChevronUpOutline14 : primitives.IconChevronDownOutline14, { size: 14, className: 'dsh-session-kit-task-stat-chevron' })
            )
          ),
          expanded ? react.createElement(
            ...
            itemLoading && (itemList === undefined || itemList.length === 0)
              ? react.createElement('div', { className: 'dsh-session-kit-task-card-item dsh-session-kit-task-card-item-loading' }, react.createElement(primitives.IconLoadingOutline16, { size: 14 }))
              : (itemList ?? []).map((item) => react.createElement(
                'div',
                { key: item.id, className: 'dsh-session-kit-task-card-item' },
                item.status === 'completed'
                  ? react.createElement(primitives.IconCheckOutline16, { size: 14, className: 'dsh-session-kit-task-card-item-check' })
            ...
            sessionLoading && (sessionList === undefined || sessionList.length === 0)
              ...
              : (sessionList ?? []).length === 0
                ...
                : (sessionList ?? []).map((session) => {
                  let title = '';
                  try { title = getSessionTitle?.(session.id) || ''; } catch { title = ''; }
                  return react.createElement(
                    'div',
                    { key: session.id, className: 'dsh-session-kit-task-card-item' },
                    react.createElement('span', { className: 'dsh-session-kit-task-session-id', title: session.id }, title || session.id),
                    title ? react.createElement('span', { className: 'dsh-session-kit-task-session-title-id' }, session.id) : null,
          ...
          (view.items ?? []).map((item) => {
            ...
            return react.createElement(
              ...
              react.createElement(
                ...
                item.status === 'completed'
                  ? react.createElement(primitives.IconCheckOutline16, { size: 14, className: 'dsh-session-kit-task-item-check' })
                ...
                }, itemExpanded ? react.createElement(primitives.IconChevronUpOutline14, { size: 14 }) : react.createElement(primitives.IconChevronDownOutline14, { size: 14 }))
              ),
              itemExpanded ? react.createElement(
                'div',
                { className: 'dsh-session-kit-task-item-detail-panel' },
                itemState.loading === true
                  ? react.createElement('div', { className: 'dsh-session-kit-task-item-panel-status' }, react.createElement(primitives.IconLoadingOutline16, { size: 14 }))
        ...
        children: react.createElement(
          ...
          react.createElement(
            ...
            react.createElement(
              ...
              search !== '' ? react.createElement('button', { type: 'button', className: 'dsh-session-kit-task-search-clear', 'aria-label': t('archiveSearchClear'), title: t('archiveSearchClear'), onClick: () => setSearch('') }, react.createElement(primitives.IconCloseOutline16, { size: 14 })) : null
            ),
      ...
      react.createElement(primitives.Modal, {
        ...
        children: react.createElement(
          ...
          react.createElement(
            ...
            trashSearch !== '' ? react.createElement('button', { type: 'button', className: 'dsh-session-kit-task-search-clear', 'aria-label': t('archiveSearchClear'), title: t('archiveSearchClear'), onClick: () => setTrashSearch('') }, react.createElement(primitives.IconCloseOutline16, { size: 14 })) : null
          ),
    ...
    function concealTurnsDelRangeGlobally(startTurn, endTurn) {
      ...
      const hideRange = (from, to) => {
        ...
        for (let index = first; index <= last; index += 1) {
          rows[index].dataset.dshTurnsDelOwner = owner;
    ...
    function apply(ctx) {
      ...
      ctx.effect(() => {
        ...
        style.textContent = `
          ...
          .dsh-session-kit-task-session-title-id { flex: none; color: var(--dsw-alias-label-tertiary); font-size: 11px; font-variant-numeric: tabular-nums; }
