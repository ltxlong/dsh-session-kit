window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function userTopicRows() {
      return Array.from(document.querySelectorAll('[data-chat-flow-kind="user"], [data-chat-flow-kind="steering"]')).filter((row) => row instanceof HTMLElement && !row.hidden && row.getClientRects().length > 0);
    }
    ...
    function rowForTopic(topic) {
      ...
      const index = typeof topic === 'object' && topic !== null && Number.isSafeInteger(topic.index) ? topic.index : undefined;
      return index === undefined ? null : userTopicRows()[index] ?? null;
    }
    ...
    function syncTopicNav(topics, setActiveKey, setLayout, setVisibleKeys) {
      if (topics.length === 0) {
        setActiveKey(null);
      ...
      const focusY = rect.top + Math.min(260, Math.max(120, (bottom - rect.top) * 0.38));
      let next = topics[0].key;
      for (const entry of rows) {
        const rowRect = entry.row.getBoundingClientRect();
        if (rowRect.top <= focusY) next = entry.topic.key;
        else break;
      }
      setActiveKey(next);
    }
    ...
    function TopicQuickNav({ useSession, t }) {
      ...
      react.useEffect(() => {
        ...
        if (button instanceof HTMLElement) button.scrollIntoView({ block: 'center' });
      ...
      useTopicLayoutEffect(() => {
        ...
        const run = () => {
          frame = 0;
          syncTopicNav(topicsRef.current, setActiveKey, setLayout, setVisibleKeys);
        };
        const schedule = () => {
          if (frame !== 0) return;
      ...
      return reactDom.createPortal(
        react.createElement(
          ...
          react.createElement(
            ...
            react.createElement(
              'div',
              { className: 'dsh-session-kit-topic-panel', onMouseLeave: () => setPanelOpen(false) },
              react.createElement(
                'ol',
    ...
    function apply(ctx) {
      ...
      ctx.effect(() => {
        ...
        style.textContent = `
          ...
          .dsh-session-kit-rename { display: flex; flex-direction: column; gap: 8px; }
          ...
          .dsh-session-kit-rename-input:focus { border-color: var(--dsw-alias-state-business-primary); box-shadow: 0 0 0 2px rgba(77, 107, 254, .14); }
