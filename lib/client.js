window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function MemoryManagementDialog({ open, t, sessionId, onClose, getSessionTitle, getCurrentUserMessage, getModelSelection, getModelDirectory, updateSidebarEntries }) {
      ...
      const [usageCompact, setUsageCompact] = react.useState(false);
      ...
      const renderUsageOverview = () => {
        ...
        return react.createElement('div', { className: 'dsh-session-kit-memory-usage' },
          ...
          usageLoading && usageOverview === null && react.createElement('div', { className: 'dsh-session-kit-archive-empty' }, t('memoryLoading')),
          ...
          (usageLoading || usageOverview !== null) && react.createElement('div', { className: 'dsh-session-kit-memory-usage-cards' },
            ...
            usageLoading && react.createElement('span', { className: 'dsh-session-kit-memory-usage-tab-spinner', role: 'status', 'aria-label': t('memoryLoading') })
          ),
        ...
        if (memoryTab === 'logs') return react.createElement('div', { className: 'dsh-session-kit-memory-logs' },
          ...
          react.createElement('div', { className: 'dsh-session-kit-memory-activity-list' },
            logItems.length === 0 ? react.createElement('div', { className: 'dsh-session-kit-archive-empty' }, t('memoryNoActivityLogs')) : logItems.map((item) => react.createElement('article', { key: item.id, className: 'dsh-session-kit-memory-activity-card' },
              react.createElement('div', { className: 'dsh-session-kit-memory-activity-head' }, react.createElement('strong', null, memoryActivityKindLabel(t, item)), react.createElement('span', { className: `dsh-session-kit-memory-badge${memoryActivityBadgeClass(item.outcome)}` }, t(`memoryActivityStatus_${item.outcome}`) || item.outcome)),
              (item.sessionTitle || item.sessionId) && react.createElement('div', { className: 'dsh-session-kit-memory-muted dsh-session-kit-memory-activity-summary' }, `${item.sessionTitle || t('memoryUnknownSession')} · ${item.sessionId || '-'}`),
              react.createElement('div', { className: 'dsh-session-kit-memory-muted dsh-session-kit-memory-activity-summary' }, item.summary || '-'),
    ...
    function DistillTurnAction({ turn, distillTurn, useSession, t }) {
      ...
      const distill = async () => {
        ...
        setError(null);
        try {
          const value = await distillTurn(turn);
          if (!alive.current) return;
          const status = value?.distill?.status;
    ...
    function apply(ctx) {
      ...
      ctx.effect(() => {
        ...
        style.textContent = `
          ...
          .dsh-session-kit-memory-usage-tab-spinner { position: absolute; left: 50%; top: 50%; width: 18px; height: 18px; box-sizing: border-box; border: 2px solid color-mix(in srgb, var(--dsw-alias-label-tertiary) 25%, transparent); border-top-color: var(--dsw-alias-state-business-primary); border-radius: 50%; transform: translate(-50%, -50%); animation: dsh-session-kit-memory-usage-spin .8s linear infinite; pointer-events: none; }
          @keyframes dsh-session-kit-memory-usage-spin { to { transform: rotate(360deg); } }
          ...
          .dsh-session-kit-memory-usage-card:disabled { cursor: wait; opacity: .72; }
          ...
          .dsh-session-kit-memory-usage-legend-label { color: var(--dsw-alias-label-primary); min-width: 0; flex: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
          ...
          .dsh-session-kit-memory-logs { display: flex; flex-direction: column; gap: 10px; min-height: 0; }
          ...
          .dsh-session-kit-memory-card-store-row { display: flex; justify-content: flex-end; align-items: center; }
