 ...
 */
async function repairSessionLogFile(filePath) {
  ...
  const strippedLines = lines.map((line) => {
    const event = JSON.parse(line);
    const { event: nextEvent, stripped } = stripRepairFields(event);
    if (stripped.length === 0) return { line, event };
  ...
  for (let i = 0; i < strippedLines.length; i += 1) {
    ...
    if (seqMap === null) { outLines.push(line); continue; }
  ...
  if (!changed) {
    return { changed: false, repairedEvents: 0, removed: {}, dropped: {} };
  }
...
async function repairCurrentSession(ctx, sessionId) {
  ...
  for (const file of files) {
    const result = await repairSessionLogFile(file);
    if (result.changed) {
...
export async function apply(ctx) {
  ...
  await installMemoryFeature(ctx).catch((error) => ctx.logger?.warn?.(`[${name}] memory feature install failed:`, error));
  ...
  ctx.effect(() => handle('/dsh-session-kit/sidebar-entries', async (req, res) => {
    ...
    if (typeof body?.memoryVisible !== 'boolean' || typeof body?.archiveVisible !== 'boolean') return respond(res, 400, { ok: false, error: 'invalid-sidebar-entries' });
    try {
      const value = { memoryVisible: body.memoryVisible, archiveVisible: body.archiveVisible };
