...
async function regenerateTurnsUnderMaintenance(ctx, agent, assistantMessageId, operationId, signal, options = {}) {
  ...
  if (prior !== undefined && priorReplayMessageId !== undefined) {
    const related = eventsSnapshot.filter((event) => isTurnsDelEvent(event) && event.data.regeneration?.operationId === operationId);
    return {
      deletion: {
...
function cachedProjectionValues(ctx, meta) {
  try {
    /* inheritedEventCount=0（未播种会话）：漏传该参数会使内部 SessionLogOffset 抛错，
       导致投影缓存永远命不中、归档列表标题回退到目录名。 */
    const block = ctx.get?.('sessionProjectionCache')?.cachedSnapshot?.(meta, 0);
