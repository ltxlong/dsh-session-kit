...
const inject = ['webServer', 'sessionPersistence', 'workspaceRegistry', 'agents', 'sessions', 'storageDomain', 'systemPrompt'];
...
async function previewArchivedSession(ctx, input) {
  ...
  const pagedUserMessages = userMessages.slice(tocOffset, tocOffset + tocLimit);
  return {
    sessionId,
