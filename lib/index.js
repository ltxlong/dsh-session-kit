...
const inject = ['webServer', 'sessionPersistence', 'workspaceRegistry', 'agents', 'sessions'];
...
const MAX_TURNS_DEL_BODY_BYTES = 16 * 1024;
...
async function regenerateTurnsUnderMaintenance(ctx, agent, assistantMessageId, operationId, signal, options = {}) {
  ...
  const prompt = userPromptForTurn(session.events, turn, targetSeq);
...
async function archivedSessionItems(ctx) {
  ...
  const metas = new Map((await ctx.sessionPersistence.list()).map((meta) => [meta.id, meta]));
  const items = [];
...
export { name, inject, TURNS_DEL_PATH, TURNS_DEL_TURN_PATH, REGENERATE_PATH, REGENERATE_TURN_PATH, TURNS_DEL_PROVIDER, TURNS_DEL_MODEL, TurnsDelError, deleteTurns, deleteTurnsByTurn, regenerateTurns, regenerateTurnsByTurn, isTurnsDelEvent };
