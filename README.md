...
`dsh-session-kit` 是一个 DeepSeek Harness 插件，用于增强会话页的日常管理能力。它不修改 DSH 核心源码，而是通过官方扩展点为会话增加管理菜单、归档会话管理、任务管理与任务档案、运行时全局提示、运行时上下文压缩配置、轮次级删除/重新生成、本地记忆管理与召回，以及右侧话题快捷导航。
...
任务管理用于记录任务从识别到完成的全过程，包括子任务、工具操作、文件观察、踩坑和涉及会话。数据与记忆共用本地 SQLite 文件：`<profile>/.dsh-session-kit/memory.sqlite`。
...
- 插件监听会话中的 `todo/write`、`tool/call`、`tool/result`、`turn/end` 和文件观察事件。
- 当前会话没有任务绑定时，第一次有效的 `todo/write` 会自动创建任务，并根据会话工作目录记录项目名。
- Todo 会按内容、名称和顺序尽量匹配已有子任务；被移出 Todo 列表的历史子任务会保留，以避免丢失已记录的操作历史。
- 任务在所有子任务完成时自动变为 `completed`；`paused` 和 `abandoned` 等人工状态不会被自动覆盖。
- 工具调用、结果和文件观察会按工具调用创建时的任务归属写入，避免在会话切换任务后污染其他任务。

#### 自动提示与完整注入

每轮开始前，插件最多为当前会话提示两个已经参与过的 `active` 任务。自动提示只包含任务名、项目、进度和已知踩坑等摘要，**不会自动把完整任务档案塞入上下文**。
...
- **新增任务**不会默认关联当前会话；只有后续注入或真实任务事件参与后才建立会话关联。
- **提取当前会话任务**会读取当前会话的最新非空 Todo 快照；同名任务只有在项目名或工作目录匹配且未进入回收站时才复用，跨项目同名任务会新建。
- 提取没有会话、事件或有效 Todo 时会显示对应原因；关闭或归档的会话会优先尝试读取持久化事件。

#### 回收站

普通删除会把任务移入回收站，而不是立即抹除任务档案。子任务、操作记录和踩坑仍会保留：

- 删除确认提示任务会在 30 天后自动清除；
- 回收站支持搜索、分页、恢复和彻底删除；
- 恢复原来处于 `active` 的任务时会变为 `paused`，避免恢复后自动继续；其他状态保持不变；
- 回收站中的任务不可编辑、不可注入，也不会参与自动识别和自动提示；
- 任务管理“全部”统计不包含回收站任务。
...
- **设置（memory_settings）**：自动蒸馏开关、全部会话启用 default、对话自动匹配项目、蒸馏模型覆盖、记忆召回模式与四段配额、分词器版本戳。
...
| 段 | 席位 | 成员 | 评分分量 |
|---|---|---|---|
| 段1 底色 | 默认 5；自定义 3~5 | 底色池 | BM25 + 自定义标签 + 画像标签 + 结构分(一档) + 时间衰减 |
| 段2 临时 | 默认 5；排除临时为 0；自定义 0~5 | 临时记忆池 | BM25 + 自定义标签 + 画像标签 |
| 段3 混合 | 默认 5；自定义 0~5 | 底色剩余 + 普通池 | 全分量（含两档结构分，防标签档位倒挂） |
| 段4 兜底 | 动态计算，至少 3 | 普通池剩余永久记忆 | 总数 − 段1 − 段2 − 段3；候选不足时吸收前三段缺口 |
...
- `lib/task.js`：任务档案采集、状态同步、任务工具、自动提示、注入、提取与回收站路由。
